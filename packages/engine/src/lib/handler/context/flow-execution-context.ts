import { ActionType, assertEqual, FlowError, FlowRunResponse, FlowRunStatus, isNil, LoopStepOutput, LoopStepResult, BaseStepOutputParams, PauseMetadata, spreadIfDefined, StepOutput, StepOutputStatus, StopResponse } from '@activepieces/shared'
...
export class FlowExecutorContext {
    ...
    public getLoopStepOutput({ stepName }: { stepName: string }): LoopStepOutput | undefined {
        ...
        if (stepOutput instanceof LoopStepOutput) {
            return stepOutput
        }
        return new LoopStepOutput(stepOutput as BaseStepOutputParams<ActionType.LOOP_ON_ITEMS, LoopStepResult>)
    }
